...
#### Example: Escape control characters in untrusted data

The pattern formatter writes log data exactly as supplied, including newlines and ANSI
color codes. When the message, tags, or named tags can contain untrusted,
attacker-controlled data, enable `escape_control_chars` to replace control characters
with a printable, escaped form (for example a newline becomes `\n` and the ANSI escape
becomes `\e`). This guards against log forging and terminal escape-sequence injection.
See [Escaping Control Characters](#escaping-control-characters) for the full rationale.
...
SemanticLogger.add_appender(
  ...
  formatter: {
    pattern: {
      pattern:              "%{time} %{level} %{name} -- %{message} %{named_tags}",
...
Only the untrusted data is escaped (message, tags, named tags, and the exception
message). The backtrace and exception class are generated by Semantic Logger and are
left untouched, so multi-line backtraces are still readable.
...
Named tags (set with `SemanticLogger.tagged(request_id: "...")`) can be pulled out
individually with `%{named_tags:key}`:
...
#### Example: A single line error format

The exception is broken into separate directives, so it can be laid out to suit a
particular log destination:

~~~ruby
SemanticLogger.add_appender(
  io:        $stdout,
  formatter: {pattern: {pattern: "%{time} %{level} %{name} -- %{message} %{exception_class}: %{exception_message}"}}
)
~~~
