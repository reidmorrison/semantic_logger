...
Adding the Bugsnag appender will send :error and :fatal log entries to Bugsnag with the error severity.
...
# Send :error and :fatal log messages to Bugsnag
SemanticLogger.add_appender(SemanticLogger::Appender::Bugsnag.new)
Rails.logger.info 'SemanticLogger Bugsnag Appender added.'

Example 2

For a non-Rails application, send :info and more severe log entries to a file called application.log and also send :error and :fatal log entries to Bugsnag.
...
class SemanticLogger::Appender::Bugsnag < SemanticLogger::Appender::Base
  ...
  def default_formatter
    proc do |log|
      { :severity => log_level(log) }.merge(Hash(log.payload))
    end
  end
  ...
  def log(log)
    ...
    return false if log.message.include?(Bugsnag::LOG_PREFIX)
